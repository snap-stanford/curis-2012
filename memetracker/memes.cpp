...
int TQuote::GetFq(const TSecTm& BegTm, const TSecTm& EndTm, const TUrlTy& UrlTy,
    const TQuoteBs& QtBs) const {
  int fq = 0;
  for (int i = 0; i < TmUrlCntV.Len(); i++) {
    if (TmUrlCntV[i].Tm() >= BegTm && TmUrlCntV[i].Tm() <= EndTm
...
int TQuote::GetUrls(const TSecTm& BegTm, const TSecTm& EndTm,
    const TUrlTy& UrlTy, const TQuoteBs& QtBs) const {
  int urls = 0;
  for (int i = 0; i < TmUrlCntV.Len(); i++) {
    if (TmUrlCntV[i].Tm() >= BegTm && TmUrlCntV[i].Tm() < EndTm
        && (UrlTy == utUndef || QtBs.GetUrlTy(TmUrlCntV[i].UrlId()) == UrlTy)) {
...
TSecTm TQuote::GetPeakTm(const TTmUnit& TmUnit, const TSecTm& AfterTm,
    TInt& FreqAtPeak) const {
...
TSecTm TQuote::GetMeanTm(const TTmUnit& TmUnit, const TUrlTy& UrlTy,
    const TQuoteBs& QtBs, const TSecTm& AfterTm) const {
  const TSecTm After = AfterTm.Round(TmUnit);
  double MeanTm = 0;
  int Cnt = 0;
  for (int i = 0; i < TmUrlCntV.Len(); i++) {
...
TSecTm TQuote::GetMedianTm(const TTmUnit& TmUnit, const TUrlTy& UrlTy,
    const TQuoteBs& QtBs, const TSecTm& AfterTm) const {
  const TSecTm After = AfterTm.Round(TmUnit);
  TMom Mom;
  for (int i = 0; i < TmUrlCntV.Len(); i++) {
...
bool TQuote::IsSinglePeak(const TTmUnit& TmUnit, const TSecTm& AfterTm,
    const TUrlTy& UrlTy, const TQuoteBs& QtBs) const {
  ...
  MaxVal *= PeakThresh;
  for (maxL = maxI; maxL > 0 && FqOtV[maxL].Val2 > MaxVal; maxL--) {
  } //printf ("  %d\t%f\n", maxL, FqOtV[maxL].Val2);}
  for (maxR = maxI; maxR < FqOtV.Len() && FqOtV[maxR].Val2 > MaxVal; maxR++) {
...
void TQuote::GetFqOt(TTmFltPrV& RawFqOtV, const TTmUnit& TmUnit,
    const TUrlTy& UrlTy, const TQuoteBs& QtBs) const {
  THash < TSecTm, TFlt > TmCntH;
  for (int i = 0; i < TmUrlCntV.Len(); i++) {
    if (UrlTy == utUndef || QtBs.GetUrlTy(TmUrlCntV[i].UrlId()) == UrlTy) {
...
void TQuote::GetFqOt(TTmFltPrV& RawFqOtV, const TTmUnit& TmUnit,
    const TSecTm& BegTm, const TSecTm& EndTm) const {
...
void TQuote::GetFqOt(TTmFltPrV& RawFqOtV, const TTmUnit& TmUnit,
    const TSecTm& BegTm, const TSecTm& EndTm, const TUrlTy& UrlTy,
    const TQuoteBs& QtBs) const {
  const TSecTm RBegTm = BegTm.Round(TmUnit);
  const TSecTm REndTm = EndTm.Round(TmUnit);
  THash < TSecTm, TFlt > TmCntH;
...
void TQuote::GetSmoothFqOt(TTmFltPrV& FqOtV, const TTmUnit& TmUnit,
    const int& WndSz, const double& Smooth, const TSecTm& BegTm,
    const TSecTm& EndTm) const {
  TTmFltPrV RawFqOtV;
  GetFqOt(RawFqOtV, TmUnit, BegTm, EndTm);
  GetSmoothFqOt(FqOtV, RawFqOtV, TmUnit, WndSz, Smooth, BegTm, EndTm);
}

void TQuote::GetSmoothFqOt(TTmFltPrV& SmoothFqOtV, const TTmFltPrV& RawFqOtV,
    const TTmUnit& TmUnit, const int& WndSz, const double& Smooth,
    const TSecTm& BegTm, const TSecTm& EndTm) {
  ...
  for (int i = 0; i < RawFqOtV.Len(); i++) {
    ...
    if (Tm < BegTm + HalfWndSecs || Tm + HalfWndSecs > EndTm) {
      NormConst = 1;
      for (int j = 1; j <= WndSz / 2; j++) {
        if (Tm >= BegTm + j * TmUnitSecs) {
          NormConst += pow(Smooth, -j);
        }
        if (Tm + j * TmUnitSecs <= EndTm) {
          NormConst += pow(Smooth, -j);
        }
      }
    }
    ...
    for (int j = 1; j <= WndSz / 2; j++) {
      ...
      if (Tm + Off <= EndTm) {
        TmCntH.AddDat(TSecTm(Tm + Off).Round(TmUnit)) += NormFq
            * pow(Smooth, -j);
      }
      if (Tm >= BegTm + Off) {
        TmCntH.AddDat(TSecTm(Tm - Off).Round(TmUnit)) += NormFq
...
void TQuote::PlotOverTm(const TStr& OutFNm) {
  ...
  for (int i = 0; i < TmUrlCntV.Len(); i++) {
    double Hr = TSecTm(TmUrlCntV[i].Tm() - TmUrlCntV[0].Tm()).Round(tmu6Hour)
        / (24 * 3600.0);
    /*for (int j = 1; j < 6; j++) {
  ...
  HrCntH.SortByKey();
  TGnuPlot::PlotValCntH(HrCntH, OutFNm,
      TStr::Fmt("%d occurences, %d urls: %s", GetFq(), GetUrls(), QtStr.CStr()),
      TStr::Fmt("Time [days] from %s", TmUrlCntV[0].Tm().GetYmdTmStr().CStr()),
      "Frequency");
}
...
/////////////////////////////////////////////////
// Quote Mutations
TQuoteBs::TQuoteBs(TSIn& SIn) :
    StrQtIdH(SIn), QuoteH(SIn), UrlInDegH(SIn), UrlTyH(SIn) {
  if (!SIn.Eof()) {
    ClustQtIdVH.Load(SIn);
  }
  TIntSet CIdSet;
  for (int q = 0; q < ClustQtIdVH.Len(); q++) {
    CIdSet.AddKey(ClustQtIdVH.GetKey(q));
    for (int c = 0; c < ClustQtIdVH[q].Len(); c++) {
      CIdSet.AddKey(ClustQtIdVH[q][c]);
...
TUrlTy TQuoteBs::GetUrlTy(const int& UrlId) const {
  if (UrlTyH.IsKey(UrlId)) {
    return (TUrlTy) UrlTyH.GetDat(UrlId).Val;
  } // return type
...
void TQuoteBs::SetUrlTy(const TStr& InFNm, const TUrlTy& SetTy) {
  ...
  for (TFIn FIn(InFNm); FIn.GetNextLn(Ln);) {
    TyUrlV.Add(Ln.GetTrunc());
  }
  ...
  for (int d = 0; d < DomUrlV.Len(); d++) {
    ...
    if (d % 1000 == 0) {
      printf("  %d/%d: labeled %d doms, %d urls [%s]\n", d, DomUrlV.Len(),
...
void TQuoteBs::GetQtIdVByFq(TIntV& QtIdV, const int& MinWrdLen,
    const int& MinQtFq, const bool& OnlyClustRoots, const TStr& HasWord,
    const TStr& AppearsAtUrl, const TUrlTy& OnlyCountTy, const TSecTm& BegTm,
    const TSecTm& EndTm) const {
  printf("Get top quotes from %d quotes\n", QuoteH.Len());
  printf("  %s -- %s\n", BegTm.GetYmdTmStr().CStr(),
      EndTm.GetYmdTmStr().CStr());
  TIntPrV FqQtIdV;
  ...
  for (int q = 0; q < QuoteH.Len(); q++) {
    ...
    if (TStrUtil::CountWords(Qt.QtStr.CStr()) < MinWrdLen) {
      words++;
      continue;
    }
    if ((!HasWord.Empty()) && Qt.QtStr.SearchStr(HasWord) == -1) {
      hasWord++;
      continue;
    }
    int Fq = 0;
    if (OnlyClustRoots && Qt.GetTy() == qtRoot) {
      ...
      IAssert(!CSet.IsKey(Qt.GetCId()));
      CSet.AddKey(Qt.GetCId());
      if ((!HasWord.Empty())
          && GetQt(GetCentrQtId(Qt.GetCId())).GetStr().SearchStr(HasWord)
              == -1) {
        hasWord++;
        continue;
      } // the quote we display has to have a word
      ...
      if (!UrlSet.Empty() && !AppearsAt(UrlSet, CentrQt)) {
        continue;
      }
      ...
      if (Doms < 3 || 5 * Doms < CentrQt.GetUrls()) {
        continue;
      }
    } else {
      if (!UrlSet.Empty() && !AppearsAt(UrlSet, Qt)) {
        continue;
      }
      ...
      if (Doms < 3 || 4 * Doms < Qt.GetUrls()) {
        continue;
      }
    }
    IAssert(!QSet.IsKey(GetQtId(q)));
    QSet.AddKey(GetQtId(q));
    FqQtIdV.Add(TIntPr(Fq, GetQtId(q)));
  }
  printf("  skip %d : word len < %d\n", words, MinWrdLen);
  printf("  skip %d : qt fq < %d\n", minFq, MinQtFq);
  if (!HasWord.Empty()) {
    printf("  skip %d : not containing '%s'\n", hasWord, HasWord.CStr());
  }
...
int AppearsAtDom(const TQuote& CentrQt, const TIntSet& GoodDom,
    const TIntH& UrlDomH) {
  int DomCnt = 0;
...
void TQuoteBs::GetQtIdVByFq(TIntV& QtIdV, const int& MinWrdLen,
    const int& MinQtFq, const TStrV& FromDomains, const bool& OnlyClustRoots,
    const TStr& HasWord, int MinDoms) const {
  ...
  for (int q = 0; q < QuoteH.Len(); q++) {
    ...
    if (Qt.GetTy() == qtRoot) {
      TQuote CentrQt;
      GetMergedClustQt(Qt.GetCId(), CentrQt, true); // get frequency only after the centroid appears
      if (AppearsAtDom(CentrQt, GoodDom, UrlDomH) < MinDoms) {
        continue;
      }
      ...
      if (AppearsAtDom(Qt, GoodDom, UrlDomH) < MinDoms) {
        continue;
      }
...
void TQuoteBs::GetQtIdVByTm(const int& WndSzHr, const int& StepHr,
    const int& MinWrdLen, const int& MinQtFq, const int& TakePerStep) const {
  const TTmUnit TmUnit = tmu4Hour;
  TSecTm MinTm, MaxTm;
  GetMinMaxTm(MinTm, MaxTm);
  MinTm = MinTm.Round(TmUnit);
  MaxTm = MaxTm.Round(TmUnit);
  printf("Dataset span %s -- %s\n", MinTm.GetStr().CStr(),
      MaxTm.GetStr().CStr());
  printf(
      "  time window %dh, step size %dh, min wrd len %d, take top %d per step\n",
  ...
  for (TSecTm Tm = MinTm; Tm <= MaxTm; Tm += StepHr * 3600, cnt++) {
    GetQtIdVByFq(TopQtIdV, MinWrdLen, MinQtFq, true, "", "", utUndef,
        TSecTm(Tm), TSecTm(Tm + StepHr * 3600));
    fprintf(F, "week of: %s\n", Tm.GetYmdTmStr().CStr());
    for (int q = 0, j = 0; q < TopQtIdV.Len() && j < 3; q++) {
      if (!TopQtIdSet.IsKey(TopQtIdV[q])) {
        fprintf(F, "%s\t%d\n", GetQt(GetCentrQtId(TopQtIdV[q])).GetStr().CStr(),
            GetClustFq(TopQtIdV[q]));
        j++;
      }
...
void TQuoteBs::GetCIdVByFq(TIntV& CIdV, const int& MinClFq,
    const TStr& RootHasWord, const TUrlTy& OnlyCountTy,
    const bool& OnlyAfterRoot, const TSecTm& BegTm, const TSecTm& EndTm) const {
  ...
  for (int c = 0; c < GetClusts(); c++) {
    ...
    if ((!RootHasWord.Empty())
        && GetQt(CentrQId).GetStr().SearchStr(RootHasWord) == -1) {
      continue;
    } // the quote we display has to have a word
    TQuote CentrQt;
    GetMergedClustQt(CId, CentrQt, OnlyAfterRoot); // get frequency only after the centroid appears
...
void TQuoteBs::GetMinMaxTm(const TIntV& QtIdV, TSecTm& MinTm,
    TSecTm&MaxTm) const {
  MinTm = MaxTm = TSecTm();
...
void TQuoteBs::GetQtPageUrl(const TIntV& QtIdV, TIntH& QtUrlIdH) const {
  ...
  for (int q = 0; q < Len(); q++) {
    const TQuote& Q = GetQtN(q);
    for (int u = 0; u < Q.GetUrls(); u++) {
      DomQtCntH.AddDat(TStrUtil::GetDomNm(GetStr(Q.TmUrlCntV[u].UrlId()))) +=
          Q.TmUrlCntV[u].Cnt();
    }
  }
  // stop-list
  const TStr StopList =
      "blog.myspace.com|www.newsmeat.com|us.rd.yahoo.com|www.startribune.com|"
          "news.originalsignal.com|uk.news.yahoo.com|ap.google.com|www.cnn.com|www.opednews.com";
  TStrV StopListV;
  StopList.SplitOnAllCh('|', StopListV);
...
void TQuoteBs::AddQuote(const TQuote& Quote, const TQuoteBs& CurQtBs) {
  ...
  for (int u = 0; u < Qt.TmUrlCntV.Len(); u++) {
    Qt.TmUrlCntV[u].SetUrlId(
        AddStr(CurQtBs.GetStr(Quote.TmUrlCntV[u].UrlId())));
  }
}
...
void TQuoteBs::AddLinks(const TVec<TChA>& LinkV, const TChA& PostUrlStr) {
  ...
  for (int l = 0; l < LinkV.Len(); l++) {
    ...
    if (TStrUtil::GetDomNm(Url) == PostDomain) {
      continue;
    } // link inside same domain
    if (IsStr(Url.CStr())) {
      UrlInDegH.AddDat(GetStrId(Url.CStr())) += 1;
    }
  }
}
...
void TQuoteBs::AddQuote2(const TChA& Quote, const TChA& PostUrlStr,
    const TSecTm& PubTm) {
  ...
  if (QtCntH.Len() > 0) {
    ...
    for (int i = 0; i < QtCntH.Len(); i++) {
      QuoteH.GetDat(QtCntH.GetKey(i)).TmUrlCntV.Add(
          TQuote::TTmUrlCnt(PubTm, PostUrlId, QtCntH[i]));
    }
  }
}

void TQuoteBs::AddQuote(const TVec<TChA>& QuoteV, const TVec<TChA>& LinkV,
    const TChA& PostUrlStr, const TSecTm& PubTm, const int& MinQtWrdLen) {
  TIntH QtCntH; // qid --> cnt
  for (int q = 0; q < QuoteV.Len(); q++) {
    if (TStrUtil::CountWords(QuoteV[q].CStr()) < MinQtWrdLen) {
      continue;
    } // skip if too short
...
int TQuoteBs::GetCentrQtId(const TIntV& ClustV) const {
  ...
  for (int c = 0; c < QtFqIdV.Len(); c++) {
    ...
    if (Words >= 6 && Words < 50
        && strstr(Q.GetStr().CStr(), FqStr.CStr()) != NULL) {
...
void TQuoteBs::GetMergedClustQt(const int& CId, TQuote& NewQt,
    const bool& OnlyAfterBegTm) const {
  const TIntV& ClustV = GetClust(CId);
  GetMergedClustQt(ClustV, NewQt, OnlyAfterBegTm);
}

void TQuoteBs::GetMergedClustQt(const TIntV& ClustV, TQuote& NewQt,
    const bool& OnlyAfterBegTm) const {
  const int CentrQtId = GetCentrQtId(ClustV);
  const TSecTm BegTm =
      OnlyAfterBegTm ? GetClustBegTm(ClustV, CentrQtId) : TSecTm(1);
  ...
  for (int c = 0; c < ClustV.Len(); c++) {
    ...
    for (int u = 0; u < Q.TmUrlCntV.Len(); u++) {
      if (Q.TmUrlCntV[u].Tm() >= BegTm) {
        TmUrlCntH.AddDat(
            TPair<TSecTm, TInt>(Q.TmUrlCntV[u].Tm(), Q.TmUrlCntV[u].UrlId())) +=
            Q.TmUrlCntV[u].Cnt();
      }
    }
  }
  NewQt.QtCIdTy = TQuote::TQtIdTy(-1, qtCentr);
  NewQt.QtStr = GetQt(CentrQtId).GetStr(); // get more frequent string
  //NewQt.QtStr = GetQt(CId).GetStr();     // get longest string
  if (!TmUrlCntH.Empty()) {
    NewQt.TmUrlCntV.Gen(TmUrlCntH.Len(), 0);
    for (int u = 0; u < TmUrlCntH.Len(); u++) {
      NewQt.TmUrlCntV.Add(
          TQuote::TTmUrlCnt(TmUrlCntH.GetKey(u).Val1,
              TmUrlCntH.GetKey(u).Val2(), TmUrlCntH[u]()));
    }
...
TSecTm TQuoteBs::GetClustBegTm(const TIntV& ClustV,
    const int& CentrQtId) const {
  TStrHash < TInt > StrH;
  TIntV CntrWIdV, WIdV;
  ...
  for (int c = 0; c < ClustV.Len(); c++) {
    ...
    if (WIdV.Len() < CntrWIdV.Len()) {
      continue;
    }
    const int Overlap = LongestCmnSubSq(CntrWIdV, WIdV, WIdV1Start, WIdV2Start,
        SkipId);
    if (Overlap >= CntrWIdV.Len() && SkipId == 0) { // centroid is contained in the bigger quote
      if (GetQt(ClustV[c]).TmUrlCntV.Empty()) {
        continue;
      }
...
int TQuoteBs::LongestCmnSubSq(const TIntV& WIdV1, const TIntV& WIdV2,
    int& WIdV1Start, int& WIdV2Start, int& SkipId) {
  ...
  for (int i = 0; i < V1Len; i++) { // word position index
    if (V2WIdSet.IsKey(V1[i])) {
      WIdPosH.AddDat(V1[i]).Add(i);
    }
  }
  // early termination
  {
    ...
    if (cmnWords < V2WIdSet.Len()) {
      if (cmnWords < 4) {
        return 0;
      } else if (V2Len >= 5 && cmnWords < 5) {
        return 0;
      } else if (V2Len > 6 && (2 * cmnWords < V2Len && cmnWords < 10)) {
        return 0;
      }
    }
  }
  ...
  for (int w = 0; w < V2Len; w++) { // short
    ...
    for (int o = 0; o < OccV.Len(); o++) {
      ...
      int cnt = 0, tmp = 0;
      while (w + cnt < V2Len && beg + cnt < V1Len
          && V2[w + cnt] == V1[beg + cnt]) {
        cnt++;
        tmp = 0;
      } // no skip
      while (beg + 1 + cnt < V1Len && w + cnt < V2Len
          && V2[w + cnt] == V1[beg + cnt + 1]) {
        cnt++;
        tmp = -1;
      } // skip word in long
      while (beg + cnt + 1 < V1Len && w + cnt + 1 < V2Len
          && V2[w + cnt + 1] == V1[beg + cnt + 1]) {
        cnt++;
        tmp = -2;
      } // skip word in both
      while (beg + cnt < V1Len && w + cnt + 1 < V2Len
          && V2[w + cnt + 1] == V1[beg + cnt]) {
        cnt++;
        tmp = -3;
      } // skip word in short
      if (MaxLen < cnt) {
...
//    if (ClustV[c].Len() < 3) {
//      continue;
//    } // skip super small clusters TODO: AHA THIS IS WHERE THE FILTERING IS TAKING PLACE!!!
    // set cluster root
...
void TQuoteBs::ClusterQts(const int& MinRootWrdLen, const int& MinQtFq,
    const TStr& OutFNmPref, const TStrV& BlackListV) {
  ...
  TVec < TPair<TInt, TIntV> > QWIdVV; // (QtId, WIdV)
  {
    ...
    for (int i = 0; i < BlackListV.Len(); i++) {
      BlackListH.AddDatId(BlackListV[i].GetTrunc());
    }
    printf("blacklist len: %d\n", BlackListH.Len());
    for (int q1 = 0; q1 < Len(); q1++) {
      const TQuote& Q = GetQtN(q1);
      const int Doms = Q.GetDoms(*this);
      if ((Q.GetTy() == qtQuote || Q.GetTy() == qtRoot) && Doms > 1
          && Doms * 4 > Q.GetUrls() && Q.GetFq() >= MinQtFq
          && (!BlackListH.IsKey(Q.GetStr().CStr()))) { // quote has not yet been merged (or is a root)
  ...
  for (int q1 = 0; q1 < ClusterQ; q1++) {
    if (QWIdVV[q1].Val2.Len() < MinRootWrdLen) {
      continue;
    } // can't be a cluster root if your length < 6
    const int Qt1Id = QWIdVV[q1].Val1;
    for (int q2 = q1 + 1; q2 < ClusterQ; q2++) {
      int idx1 = 0, idx2 = 0, SkipTy = 0;
      bool DoMerge = false;
      const int ShortLen = TMath::Mn(QWIdVV[q1].Val2.Len(),
          QWIdVV[q2].Val2.Len());
      if (ShortLen == 0) {
        continue;
      }
      const int Overlap = LongestCmnSubSq(QWIdVV[q1].Val2, QWIdVV[q2].Val2,
          idx1, idx2, SkipTy);
      if (ShortLen == 4 && Overlap == 4 /*&& SkipTy==0*/) {
        DoMerge = true;
      } // full overlap, no skip
      else if (ShortLen == 5 && Overlap == 5 /*&& SkipTy==0*/) {
        DoMerge = true;
      } // full overlap, no skip
      else if ((ShortLen == 6 && Overlap >= 5 /*&& SkipTy==0) || (ShortLen == 6 && Overlap == 5*/)) {
        DoMerge = true;
      } else if (Overlap / double(ShortLen + 3) > 0.5 || Overlap > 10) {
        DoMerge = true;
      }
      if (DoMerge == true) {
        NMergers++;
        ...
        // save
        {
          ...
          if (ClustQtIdVH.GetDat(Qt1Id).Len() == 2) {
            fprintf(F, "\n[%d] %s \t%d\n",
                TStrUtil::CountWords(Q1.QtStr.CStr()), Q1.QtStr.CStr(),
                Q1.GetFq());
          }
          fprintf(F, "%s%d [%d] %s \t%d\n", Str.CStr(), Overlap,
              TStrUtil::CountWords(Q2.QtStr.CStr()), Q2.QtStr.CStr(),
              Q2.GetFq());
        }
      }
    }
    ...
    if (q1 > 0 && q1 % 100 == 0) {
      printf("\r  %d/%d: %d merged [%s]  ", q1, ClusterQ, NMergers,
          ExeTm.GetStr());
      if (q1 % 1000 == 0) {
        printf("save.\n");
        fflush(F);
        TFOut FOut(OutFNmPref + "-QtBs.ClustQtIdVH");
        ClustQtIdVH.Save(FOut);
      }
    }
  }
  fclose(F);
  printf("\n%d quotes, %d clusters [%s]\n\n.", NMergers, ClustQtIdVH.Len(),
      ExeTm.GetStr());
}
...
void TQuoteBs::Mergec2Clusters(const int& ParentCId, const int& ChildCId) {
  ...
  printf("merging %d (fq: %d) to %d (%d fq)\n", ChildCId, GetClustFq(ChildCId),
      ParentCId, GetClustFq(ParentCId));
  TIntSet ClustSet;
  {
...
PClustNet TQuoteBs::GetClustNet(const int& MinQtFq,
    const TStr& OutFNmPref) const {
  ...
  THash < TInt, TIntV > QWIdVH; // QtId --> WIdV
  {
    ...
    for (int q1 = 0; q1 < Len(); q1++) {
      const TQuote& Q = GetQtN(q1);
      const int Doms = Q.GetDoms(*this);
      if ((Q.GetTy() != qtCentr) && Doms > 1 && Doms * 4 > Q.GetUrls()
          && Q.GetFq() >= MinQtFq) { // quote has not yet been merged (or is a root)
        LenQIdV.Add(TIntPr(TStrUtil::CountWords(Q.QtStr.CStr()), GetQtId(q1)));
      }
    }
  ...
  for (int q1 = 0; q1 < ClusterQ; q1++) { // for each quote
    ...
    for (int q2 = q1 - 1; q2 >= 0; q2--) { //  go over all shorter quotes
      ...
      if (SeenQtIdSet.IsKey(Qt2Id)) {
        continue;
      }
      bool DoMerge = false;
      const int Overlap = LongestCmnSubSq(QWIdVH[q1], QWIdVH[q2], idx1, idx2,
          SkipTy);
      const int ShortLen = QWIdVH[q1].Len();
      IAssert(QWIdVH[q1].Len() <= QWIdVH[q2].Len());
      if (ShortLen == 4 && Overlap == 4) {
        DoMerge = true;
      } // full overlap, no skip
      else if (ShortLen == 5 && Overlap == 5) {
        DoMerge = true;
      } // full overlap, no skip
      else if (ShortLen == 6 && Overlap >= 5) {
        DoMerge = true;
      } else if (Overlap / double(ShortLen + 3) > 0.5 || Overlap > 10) {
        DoMerge = true;
      }
      if (DoMerge == true) {
        NMerges++;
        if (!ClustNet->IsNode(Qt1Id)) {
          ClustNet->AddNode(Qt1Id, GetQt(Qt1Id));
        }
        if (!ClustNet->IsNode(Qt2Id)) {
          ClustNet->AddNode(Qt2Id, GetQt(Qt2Id));
        }
        //ClustNet->AddEdge(Qt1Id, Qt2Id, -1, Overlap);
        ClustNet->AddEdge(Qt1Id, Qt2Id);
        SeenQtIdSet.AddKey(Qt1Id);
        SeenQtIdSet.AddKey(Qt2Id);
        ...
        fprintf(F, "%d Merge\t%d\t%d\ttree\t%d\n\t%s\n\t%s\n", q1,
            QWIdVH[q1].Len(), QWIdVH[q2].Len(), NIdDepthH.Len(),
            GetQt(Qt2Id).GetStr().CStr(), GetQt(Qt1Id).GetStr().CStr());
        fflush(F);
      }
    }
    if (q1 > 0 && q1 % 100 == 0) {
      printf("\r  %d/%d: %d merged [%s]  ", q1, ClusterQ, NMerges,
          ExeTm.GetStr());
      if (q1 % 1000 == 0) {
        printf("save: %d merges, %d nodes, %d edges in ClustNet [%s]\n.",
            NMerges, ClustNet->GetNodes(), ClustNet->GetEdges(),
            ExeTm.GetStr());
        TFOut FOut(OutFNmPref + ".ClustNet");
        ClustNet->Save(FOut);
      }
    }
  }
  ...
  printf("\n%d merges, %d nodes, %d edges in ClustNet [%s]\n\n.", NMerges,
      ClustNet->GetNodes(), ClustNet->GetEdges(), ExeTm.GetStr());
  return ClustNet;
}
...
void TQuoteBs::GetTopQtDoms(TStrIntPrV& DomCntV, const int& TakeNClust,
    const int& MinClFq, const TStr& RootHasWord,
    const TUrlTy& OnlyCountTy) const {
  TIntV CIdV;
...
void TQuoteBs::TopDomsByLag(const TTmUnit& TmUnit, const int& TakeNDoms,
    const int& TakeNClusts, const int& Thresh) const {
  THash < TStr, TMom > DomLagH;
  TIntV CIdV;
  GetCIdVByFq(CIdV, 10, "", utUndef);
  //TIntV QIdV;  GetQIdVByFq(QIdV, 10, "", utUndef);
  TStrIntPrV DomCntV;
  GetTopQtDoms(DomCntV, TakeNClusts, 10, "", utUndef);
  ...
  for (int c = 0; c < TMath::Mn(CIdV.Len(), TakeNClusts); c++) {
    TQuote Q;
    GetMergedClustQt(CIdV[c], Q, false);
    ...
    for (int u = 0; u < Q.TmUrlCntV.Len(); u++) {
      ...
      if (DomSet.IsKey(Dom)) {
        continue;
      } // only take first query occurence on the domain
      if (!DomLagH.IsKey(Dom)) {
        continue;
      }
      DomLagH.AddDat(Dom).Add(
          (int(Q.TmUrlCntV[u].Tm().Round(TmUnit)) - int(MedTm)) / 3600.0);
      DomSet.AddKey(Dom);
    }
  ...
  for (int i = 0; i < DomLagH.Len(); i++) {
    DomLagH[i].Def();
    MedLagDomV.Add(
        TFltFltStrTr(DomLagH[i].GetMedian(), DomLagH[i].GetVals(),
            DomLagH.GetKey(i)));
  }
  MedLagDomV.Sort();
  fprintf(F,
  ...
  for (int i = 0; i < MedLagDomV.Len(); i++) {
    if (MedLagDomV[i].Val2() < Thresh * TakeNClusts / 100.0) {
      continue;
    }
    fprintf(F, "%g\t%g\t%s\n", MedLagDomV[i].Val1(), MedLagDomV[i].Val2(),
        MedLagDomV[i].Val3.CStr());
  }
...
void TQuoteBs::PlotQtFqCnt(const TStr& OutFNmPref) const {
  ...
  TGnuPlot::PlotValCntH(UrlCntH, OutFNmPref + "-url", "",
      "number of urls quote appears at", "count", gpsLog10XY);
  TGnuPlot::PlotValCntH(DomCntH, OutFNmPref + "-dom", "",
      "number of domains quote appears at", "count", gpsLog10XY);
  TGnuPlot::PlotValCntH(CntFqH, OutFNmPref + "-fq", "",
      "number of times quote appears", "count", gpsLog10XY);
}

void TQuoteBs::PlotQtMediaVsBlogFq(const int& QtId,
    const TStr& OutFNmPref) const {
  ...
  TGnuPlot GP("mediaVsBlogs-" + OutFNmPref,
      TStr::Fmt("%s. b:%d m:%d u:%d d:%d", ClQt.GetStr().CStr(),
          ClQt.GetFq(utBlog, *this), ClQt.GetFq(utMedia, *this), ClQt.GetUrls(),
          ClQt.GetDoms(*this)));
  TQuote::GetSmoothFqOt(SmoothV, BlogFqV, tmu4Hour, 24, 1.2, MinTm);
  for (int i = 0; i < SmoothV.Len(); i++) {
    FqV.Add(
        TFltPr(double(SmoothV[i].Val1 - MinTm) / (24.0 * 3600.0),
            SmoothV[i].Val2()));
  }
  ...
  for (int i = 0; i < SmoothV.Len(); i++) {
    ...
    FqV.Add(
        TFltPr(
            double(SmoothV[i].Val1 - MinTm) / (24.0 * 3600.0) * (54.0 / 46.0),
            SmoothV[i].Val2()));
  }
  GP.AddPlot(FqV, gpwLines, "Media frequency");
  GP.SetXYLabel(TStr::Fmt("time [days] since %s", MinTm.GetYmdTmStr().CStr()),
      "Quote frequency (normalized for the baseline 46 vs 54)");
  GP.SavePng();
}

int NormMaxTo1(const TQuote::TTmFltPrV& TmFqV, TFltPrV& OutV,
    const TSecTm& BegTm) {
  int Idx = 0;
  double MaxVal = 0;
  ...
  for (int i = 0; i < TmFqV.Len(); i++) {
    OutV.Add(
        TFltPr(double(TmFqV[i].Val1 - BegTm) / (24.0 * 3600.0),
            TmFqV[i].Val2())); //MaxVal));
  }
  return Idx;
}

void TQuoteBs::PlotClustMediaVsBlogFq(const int& CId,
    const TStr& OutFNmPref) const {
  const TTmUnit TmUnit = tmu4Hour;
  ...
  const int Lag = (int(BlogPeak.Round(TmUnit)) - int(MediaPeak.Round(TmUnit)))
      / 3600;
  const bool Is1Peak = ClQt.IsSinglePeak(TmUnit, TSecTm(1), utUndef, *this);
  TGnuPlot GP("mediaVsBlogs-" + OutFNmPref,
      TStr::Fmt("%s. b:%d m:%d u:%d d:%d",
          Is1Peak ? "SINGLE PEAK" : "MORE PEAKS", //ClQt.GetStr().CStr(),
          Lag, ClQt.GetFq(utBlog, *this), ClQt.GetFq(utMedia, *this),
          ClQt.GetUrls(), ClQt.GetDoms(*this)));
  // blogs
  ...
  GP.AddPlot(FqV, gpwLines,
      TStr::Fmt("SMOOTH Blog : max %s", SmoothV[mx].Val1.GetYmdTmStr().CStr()),
      "pt 7 ps 1");
  ...
  const int SmLag = (int(SmBlog.Round(TmUnit)) - int(SmMed.Round(TmUnit)))
      / 3600;
  GP.AddPlot(FqV, gpwLines,
      TStr::Fmt("SMOOTH Media is behind: %dh, smooth: %dh.", -Lag, -SmLag),
      "pt 5 ps 1");
  //ClQt.GetFqOt(AllFqV, TmUnit);
  //NormMaxTo1(AllFqV, FqV, MinTm);
  //GP.AddPlot(FqV, gpwLines, "ALL", "pt 5 ps 1");
  GP.SetXYLabel(TStr::Fmt("time [days] since %s", MinTm.GetYmdTmStr().CStr()),
      "Cluster frequency (normalized for the baseline 46 vs 54)");
  GP.AddCmd("set xtics 10");
  GP.AddCmd("set mxtics 10");
  GP.SavePng();
}

void TQuoteBs::PlotMediaVsBlogLag(const TTmUnit& TmUnit,
    const bool& TakeClusters, const int& TakeN, const TStr& OutFNmPref) const {
  TQuote Qt;
  TIntV IdV;
  ...
  int Cnt = 0;
  if (TakeClusters) {
    GetCIdVByFq(IdV, 10, "", utUndef, false);
  } else {
    GetQtIdVByFq(IdV, 8, 10, false, "", "", utUndef);
  }
  for (int c = 0; c < TakeN; c++) {
    if (TakeClusters) {
      GetMergedClustQt(IdV[c], Qt, false);
    } else {
      Qt = GetQt(IdV[c]);
    }
    ...
    const double Lag = (int(BlogPeak.Round(TmUnit))
        - int(MediaPeak.Round(TmUnit))) / 3600.0;
    const double Lag1 = (int(BlogPeak.Round(TmUnit))
        - int(AllPeak.Round(TmUnit))) / 3600.0;
    const double Lag2 = (int(MediaPeak.Round(TmUnit))
        - int(AllPeak.Round(TmUnit))) / 3600.0;
    if (fabs(Lag) < 1 * 24) {
      LagMom1d.Add(Lag);
    }
  ...
  LagMom7d.Def();
  {
    TGnuPlot GP("lagBlogsVsMedia-" + OutFNmPref,
        TStr::Fmt(
            "Lag: 1D: %.2f %g;   2D: %.2f  %g;   3D: %.2f  %g;   4D: %.2f  %g;   7D: %.2f  %g",
            LagMom1d.GetMean(), LagMom1d.GetMedian(), LagMom2d.GetMean(),
            LagMom2d.GetMedian(), LagMom3d.GetMean(), LagMom3d.GetMedian(),
            LagMom4d.GetMean(), LagMom4d.GetMedian(), LagMom7d.GetMean(),
            LagMom7d.GetMedian()), true);
    LagBCntH.GetKeyDatPrV(PrV);
    PrV.Sort();
    GP.AddPlot(PrV, gpwLinesPoints,
        TStr::Fmt("%s. b:%d m:%d u:%d d:%d BLOG", Qt.GetStr().CStr(),
            Qt.GetFq(utBlog, *this), Qt.GetFq(utMedia, *this), Qt.GetUrls(),
            Qt.GetDoms(*this)));
    LagMCntH.GetKeyDatPrV(PrV);
    PrV.Sort();
    GP.AddPlot(PrV, gpwLinesPoints,
        TStr::Fmt("%s. b:%d m:%d u:%d d:%d MEDIA", Qt.GetStr().CStr(),
            Qt.GetFq(utBlog, *this), Qt.GetFq(utMedia, *this), Qt.GetUrls(),
            Qt.GetDoms(*this)));
  ...
  printf("\nTake top %d clusters. TmUnit: %s\n", TakeN,
      TTmInfo::GetTmUnitStr(TmUnit).CStr());
...
void TQuoteBs::PlotFqDecay(const TTmUnit& TmUnit, const bool& TakeClusters,
    const TUrlTy& CntUrlTy, const int& PlotN, const int& MinValsPerTm,
    const TStr& OutFNmPref) const {
  ...
  if (TakeClusters) {
    GetCIdVByFq(IdV, 10, "", CntUrlTy, false);
  } else {
    GetQtIdVByFq(IdV, 8, 10, false, "", "", CntUrlTy);
  }
  for (int c = 0; c < PlotN; c++) {
    ...
    for (int i = 0; i < QtV.Len(); i++) {
      TmV.Add(
          TFltPr((double(QtV[i].Val1) - double(PeakTm)) / (24.0 * 3600),
              QtV[i].Val2 / PeakVal));
  ...
  for (int i = 0; i < MomH.Len(); i++) {
    MomH[i].Def();
    if (MomH[i].GetVals() < MinValsPerTm) {
      printf(".");
      continue;
    }
  ...
  GP.AddPlot(MedV, gpwLines,
      TStr::Fmt("Median (last qt: %d %d %d)", Qt.GetFq(), Qt.GetUrls(),
          Qt.GetDoms(*this)), "lt 1 lw 2");
...
void TQuoteBs::PlotBlogVsMediaFqOt(const TTmUnit& TmUnit,
    const bool& TakeClusters, int PlotN, const TStr& OutFNmPref) const {
  ...
  TInt FqAtPeak;
  if (TakeClusters) {
    GetCIdVByFq(IdV, 100, "", utUndef, false);
  } else {
    GetQtIdVByFq(IdV, 8, 10, false, "", "", utUndef);
  }
  for (int c = 0; c < TMath::Mn(PlotN, IdV.Len()); c++) {
    ...
    if (!Qt.IsSinglePeak(TmUnit, TSecTm(0), utUndef, *this)) {
      PlotN++;
      printf(".");
      continue;
    }
    TQuote::TTmFltPrV AllFqV, BlogFqV, MedFqV;
    TFltPrV TmV;
    ...
    for (int i = 0; i < MedFqV.Len(); i++) {
      PeakFq = TMath::Mx((double) MedFqV[i].Val2(), PeakFq);
    }
    for (int i = 0; i < MedFqV.Len(); i++) {
      const int T1 = (int) TMath::Round(
          (double(MedFqV[i].Val1.GetAbsSecs()) - Peak1) / (3600.0));
      const int T2 = (int) TMath::Round(
          (double(MedFqV[i].Val1.GetAbsSecs()) - Peak2) / (3600.0));
      const int T3 = (int) TMath::Round(
          (double(MedFqV[i].Val1.GetAbsSecs()) - Peak3) / (3600.0));
    ...
    for (int i = 0; i < BlogFqV.Len(); i++) {
      PeakFq = TMath::Mx((double) BlogFqV[i].Val2(), PeakFq);
    }
    for (int i = 0; i < BlogFqV.Len(); i++) {
      const int T1 = (int) TMath::Round(
          (double(BlogFqV[i].Val1.GetAbsSecs()) - Peak1) / (3600.0));
      const int T2 = (int) TMath::Round(
          (double(BlogFqV[i].Val1.GetAbsSecs()) - Peak2) / (3600.0));
      const int T3 = (int) TMath::Round(
          (double(BlogFqV[i].Val1.GetAbsSecs()) - Peak3) / (3600.0));
  ...
  NormPeakAt1(Peak1AllH);
  NormPeakAt1(Peak2AllH);
  NormPeakAt1(Peak3AllH);
  NormPeakAt1(Peak1MedH);
  NormPeakAt1(Peak2MedH);
  NormPeakAt1(Peak3MedH);
  NormPeakAt1(Peak1BlogH);
  NormPeakAt1(Peak2BlogH);
  NormPeakAt1(Peak3BlogH);
  TGnuPlot::PlotValCntH(Peak1AllH, "ALL", Peak1MedH, "MEDIA", Peak1BlogH,
      "BLOG", "peakMax-" + OutFNmPref, "Max is the peak", "Time [h]",
      "Count (sum of frequencies of all quotes)");
  ...
  TGnuPlot::PlotValCntH(Peak3AllH, "ALL", Peak3MedH, "MEDIA", Peak3BlogH,
      "BLOG", "peakMed-" + OutFNmPref, "Median is the peak", "Time [h]",
      "Count (sum of frequencies of all quotes)");
...
void TQuoteBs::PlotBlogFracOt(const TTmUnit& TmUnit, const bool& TakeClusters,
    const int& PlotN, const TStr& OutFNmPref) const {
  TQuote Qt;
  TIntV IdV;
  ...
  for (int c = 0; c < PlotN; c++) {
    ...
    for (int m = 0, b = 0; m < MedQtV.Len(); m++) {
      while (b < BlgQtV.Len() && BlgQtV[b].Val1 < MedQtV[m].Val1) {
        b++;
      }
      if (b < BlgQtV.Len() && BlgQtV[b].Val1 == MedQtV[m].Val1) {
        const double Tm = (int(MedQtV[m].Val1.Round(TmUnit)) - PeakTm)
            / (24 * 3600.0);
        //if (BlgQtV[b].Val2>1 && MedQtV[m].Val2>1) {
        const double Frac = BlgQtV[b].Val2
            / double(BlgQtV[b].Val2 + MedQtV[m].Val2);
  ...
  }
  {
    ...
    for (int b = 0; b < TmBmH.Len(); b++) {
      PrV.Add(
          TFltPr(TmBmH.GetKey(b),
              TmBmH[b].Val1 / (TmBmH[b].Val1 + TmBmH[b].Val2)));
    }
...
void TQuoteBs::PlotPopularityCnt(const bool& TakeClusters, const TUrlTy& UrlTy,
    const int& PlotN, const TStr& OutFNmPref) const {
  TIntV IdV;
  TIntH FqCntH;
  TQuote Qt;
  ...
  TIntPrV FqCntV, CdfV;
  FqCntH.GetKeyDatPrV(FqCntV);
  FqCntV.Sort();
  TGUtil::GetCCdf(FqCntV, CdfV);
  TGnuPlot::PlotValV(FqCntV, "pop-" + OutFNmPref, "", "total frequency",
      "count", gpsLog10XY);
  TGnuPlot::PlotValV(CdfV, "popCDF-" + OutFNmPref + "", "", "total frequency",
      "NCDF", gpsLog10XY);
}

void TQuoteBs::PlotEmptyY(const TTmUnit& TmUnit, const bool& TakeClusters,
    const int& PlotN, const TStr& OutFNmPref) const {
  TQuote Qt;
  TIntV IdV;
  ...
  TGnuPlot::PlotValCntH(EmptyH, "emptyF-" + OutFNmPref + "-d",
      "Amount of empty area above the cummulative query-frequency curve",
      "y value", "area at y");
  TGnuPlot::PlotValV(EmptyV, "emptyF-" + OutFNmPref + "-c",
      "Amount of empty area above the cummulative query-frequency curve",
      "y value", "area above y");
  TGnuPlot::PlotValV(TmV, "emptyF1" + OutFNmPref,
      "Cummulative query frequency over time", "time (days)", "frequency");
}
    ...
    if (Q.GetTy() == qtRoot || Q.GetTy() == qtCentr) { // cluster root or centroid
      ...
      for (int i = 0; i < ClustV.Len(); i++) {
        ...
        fprintf(F, "\t[%d] %d=%d %s\t%d\t%d\t%d\n",
            ...
            ClustV[i](), Q1.GetCId(), Q1.QtStr.CStr(), Q1.GetFq(), Q1.GetUrls(),
            Q1.GetDoms(*this));
...
void TQuoteBs::SaveClusters(const TStr& OutFNm, const bool& SkipUrls) const {
  ...
  fprintf(stdout, "Saving %d clusters\n", FqCIdV.Len());
  for (int c = 0; c < FqCIdV.Len(); c++) {
    const int CId = FqCIdV[c].Val2;
    const TIntV& ClustV = GetClust(CId);
    fprintf(F, "\n%d\t%d\t%s\t%d\n", ClustV.Len(), FqCIdV[c].Val1(),
        GetQt(CId).GetStr().CStr(), CId);
    for (int q = 0; q < ClustV.Len(); q++) {
      ...
      fprintf(F, "\t%d\t%d\t%s\t%d\n", Qt.GetFq(), Qt.GetUrls(),
          Qt.GetStr().CStr(), ClustV[q]());
      if (!SkipUrls) {
        for (int u = 0; u < Qt.GetUrls(); u++) {
          fprintf(F, "\t\t%s\t%d\t%s\t%s\n",
              Qt.TmUrlCntV[u].Tm().GetYmdTmStr().CStr(), Qt.TmUrlCntV[u].Cnt(),
              GetUrlTy(Qt.TmUrlCntV[u].UrlId()) == utMedia ? "M" : "B",
              GetStr(Qt.TmUrlCntV[u].UrlId()));
        }
      }
    }
  }
...
// save: root qt, all members, all urls tms for each memeber
void TQuoteBs::SaveClusters(const TIntV& QtIdV, const TStr& OutFNm,
    const bool& SkipUrls) const {
  FILE *F = fopen(TStr::Fmt("%s-clust.txt", OutFNm.CStr()).CStr(), "wt");
  fprintf(F, "format:\n<ClSz>\t<TotFq>\t<Root>\t<ClusterId>\n");
  fprintf(F, "\t<QtFq>\t<Urls>\t<QtStr>\t<QuteId>\n");
  fprintf(F, "\t\t<Tm>\t<Fq>\t<UrlTy>\t<Url>\n\n");
  for (int c = 0; c < QtIdV.Len(); c++) {
    int CId = QtIdV[c];
    if (!IsClust(CId)) {
      CId = GetQt(CId).GetCId();
    }
    const TIntV& ClustV = GetClust(CId);
    fprintf(F, "\n%d\t%d\t%s\t%d\n", ClustV.Len(), GetClustFq(CId),
        GetQt(CId).GetStr().CStr(), CId);
    for (int q = 0; q < ClustV.Len(); q++) {
      const TQuote& Qt = GetQt(ClustV[q]);
      if ((!SkipUrls) && q > 0) {
...
void TQuoteBs::SaveBigBlogMassQt(const TStr& OutFNm) const {
  ...
  for (int c = 0; c < IdV.Len(); c++) {
    ...
    const double BlogFq = Qt.GetUrls(TSecTm(MediaPeak - 21 * 24 * 3600),
        TSecTm(MediaPeak - 7 * 24 * 3600), utBlog, *this); // Fq 7 week before the media peak
    ...
    Candidates++;
    if (BlogFq < TotFq * 0.15 || BlogFq > TotFq * 0.75) {
      continue;
    }
    //if (Score > 0.5) { continue; }
    ScoreV.Add(
        TFltStrPr(TotFq,
            TStr::Fmt("%g\t%g\t%s", BlogFq, TotFq, Qt.GetStr().CStr())));
    /*if (TotFq > 500) {
  ...
  fprintf(F,
      "counts are numbers of urls (not actually number of occurences)\n");
  fprintf(F, "\nblog\ttotal\tQuote\n");
...
void TQuoteBs::SaveForFlash(const TIntV& QtIdV, const TStr& OutFNm,
    const TTmUnit& TmUnit, int SaveN, const TSecTm& BegTm, const TSecTm& EndTm,
    const TUrlTy& OnlyCountTy) const {
  const int TmUnitSecs = TTmInfo::GetTmUnitSecs(TmUnit);
  ...
  for (int q = 0; q < QtIdV.Len() && QtInfoV2.Len() < (SaveN + 100); q++) { // take top SaveN
    ...
    if (Qt.GetTy() == qtQuote) {
      ...
      QtInfoV2.Add(
          TQuad<TFlt, TInt, TInt, TQuote::TTmFltPrV>(
              (double) Qt.GetPeakTm(TmUnit, BegTm) + 1e-6 * Qt.GetUrls(),
              QtIdV[q], Qt.GetFq(), FqV));
      ...
      if (!ClustQtIdVH.IsKey(CId)) {
        continue;
      }
      const int CentrQtId = GetCentrQtId(CId);
      TQuote CentrQt;
      GetMergedClustQt(CId, CentrQt, true);
      ...
      QtInfoV2.Add(
          TQuad<TFlt, TInt, TInt, TQuote::TTmFltPrV>(
              (double) CentrQt.GetPeakTm(TmUnit, BegTm)
                  + 1e-6 * CentrQt.GetUrls(), CentrQtId, GetClustFq(CId), FqV));
      TInt PeakFq;
  ...
  for (int i = 0; i < QtInfoV.Len(); i++) {
    ...
    int x = 0;
    for (x = 0; x < FqV.Len() && FqV[x].Val1 <= LastTime + Slack; x++) {
    }
    if (x < FqV.Len()) {
      FqV.Del(x, FqV.Len() - 1);
    }
  ...
  QtInfoV.Sort();
  TIntH QtIdUrlH;
  GetQtPageUrl(VizQtIdV, QtIdUrlH);
  // save quote info
  ...
  for (int i = 0; i < TMath::Mn(SaveN, QtInfoV.Len()); i++) {
    fprintf(F, "%d\t%s\t%s\t%d\n", SaveN - i,
        GetQt(QtInfoV[i].Val2).QtStr.CStr(),
        GetStr(QtIdUrlH.GetDat(QtInfoV[i].Val2)), (int) QtInfoV[i].Val3);
  }
  fclose(F);
  printf("saveT\n");
  ...
  for (int i = 0; i < TMath::Mn(SaveN, QtInfoV.Len()); i++) {
    ...
    TQuote::GetSmoothFqOt(SmoothFqV, RawFqV, TmUnit, 6 * 24 * 3600 / TmUnitSecs,
        1.05, MinTm, MaxTm); // more smootning
    //SmoothFqV = RawFqV;
    IAssert(SmoothFqV.Len() >= RawFqV.Len());
    for (int d = 0, ds = 0; ds < SmoothFqV.Len(); ds++) {
      if (TmUnit == tmuDay) {
        fprintf(F, "%s", SmoothFqV[ds].Val1.GetDtYmdStr().CStr());
      } else {
        fprintf(F, "%s_%02d", SmoothFqV[ds].Val1.GetDtYmdStr().CStr(),
            SmoothFqV[ds].Val1.GetHourN());
      }
      fprintf(F, "\t%d\t%.2f", SaveN - i, SmoothFqV[ds].Val2());
      if (d < RawFqV.Len() && SmoothFqV[ds].Val1 == RawFqV[d].Val1) {
        fprintf(F, "\t%.0f", RawFqV[d].Val2());
        d++;
      } else {
        fprintf(F, "\t0");
      }
...
void TQuoteBs::SaveDomainStat(const TStr& OutFNm, const int& MinCnt) const {
  ...
  for (int q = 0; q < QuoteH.Len(); q++) {
    ...
    for (int u = 0; u < Q.GetUrls(); u++) {
      ...
      AllOt.AddDat(Day) += 1;
      AllCnt++;
      if (GetUrlTy(Q.GetUrlId(u)) == utMedia) {
        MedOt.AddDat(Day) += 1;
        MedCnt++;
      } else {
        BlogOt.AddDat(Day) += 1;
  ...
  TGnuPlot::PlotValCntH(AllOt, "ALL", MedOt, "MEDIA", BlogOt, "BLOGS",
      "overTm-" + OutFNm,
      TStr::Fmt("Quote freq over time: A:%d M:%d B:%d", AllCnt, MedCnt,
          BlogCnt), "Time [days]", "Number of quote mentions");
  DomCntH.SortByDat(false);
  FILE *F = fopen(TStr::Fmt("domains-%s.tab", OutFNm.CStr()).CStr(), "wt");
  fprintf(F, "# %d domains, total mentions:\tAll:%d\tMedia:%d\tBlogs:%d",
      DomCntH.Len(), AllCnt, MedCnt, BlogCnt);
  fprintf(F, "#Domain\tNumber of quotes mentioned\tNumber of mentions\n");
  for (int d = 0; d < DomCntH.Len(); d++) {
    fprintf(F, "%s\t%d\t%d\n", DomCntH.GetKey(d).CStr(), DomCntH[d].Val1(),
        DomCntH[d].Val2());
  }
  fclose(F);
}
...
void TQuoteBs::Dump(const bool& Fast) const {
  ...
  if (!Fast) {
    TSecTm MinTm, MaxTm;
    GetMinMaxTm(MinTm, MaxTm);
    printf("  %s -- %s\n", MinTm.GetYmdTmStr().CStr(),
        MaxTm.GetYmdTmStr().CStr());
  }
  printf("  %d quotes\n", Len());
...
PQuoteBs TQuoteBs::FindQtInTxtQBs(const TStr& InBinFNmWc,
    const TStrV& QuoteStrV) {
  ...
  for (int q = 0; q < QuoteStrV.Len(); q++) {
    ...
    for (int w = 1; w < WIdV.Len(); w++) {
      int skipLen = 1;
      while (w - skipLen >= 0 && WIdV[w - skipLen] != WIdV[w]) {
        skipLen++;
      }
      SkipV.Add(skipLen);
    }
  ...
  while (QL.Next()) {
    if (SeenUrlH.IsKey(TMd5Sig(QL.PostUrlStr))) {
      continue;
    }
    ...
    WrdV.Clr(false);
    {
      ...
      for (int w = 0; w < WrdV.Len(); w++) {
        if (StrH.IsKeyGetDat(WrdV[w], WId)) {
          TxtWIdV.Add(WId);
        } else if (TxtWIdV.Len() > 0 && TxtWIdV.Last() != -1) {
          TxtWIdV.Add(-1);
        }
      }
    }
    ...
    for (int q = 0; q < QtWIdVV.Len(); q++) {
      ...
      for (int w = 0; w < TxtWIdV.Len() - QWIdV.Len();) {
        int len = 0;
        while (len < QWIdV.Len() && TxtWIdV[w + len] == QWIdV[len]) {
          len++;
        }
        if (len == QWIdV.Len()) {
          cnt += 1;
      ...
      if (cnt > 0) {
        QtBs.GetQtN(q).TmUrlCntV.Add(
            TQuote::TTmUrlCnt(QL.PubTm, QtBs.AddStr(QL.PostUrlStr), cnt));
        if (cnt > 255) {
          printf("*");
        }
      }
  ...
  for (int i = 0; i < QtBs.Len(); i++) {
    QtBs.QuoteH[i].TmUrlCntV.Sort();
  }
  return QtBsPt;
}
...
TMakeQtBsFromSpinn3r::TMakeQtBsFromSpinn3r(const PQuoteBs& QtBsPt,
    const int& MinWordLen) :
    TQuoteExtractor(false), QtBs(QtBsPt), MinWrdLen(MinWordLen) { // don't save post content in .ContentBin
}
...
void TMakeQtBsFromSpinn3r::SavePlots() {
  ...
  TStr TmStr =
      TStr::Fmt("%s -- %s",
          TSecTm(3600 * ItmPerH.GetKey(0) + BegTm).GetYmdTmStr().CStr(),
          TSecTm(3600 * ItmPerH.GetKey(ItmPerH.Len() - 1) + BegTm).GetYmdTmStr().CStr());
  TGnuPlot::PlotValCntH(WrdPerH, "overTm-WordsPerH",
      TStr("Words per hour.") + TmStr, "Time [hours]", "Total words");
  TGnuPlot::PlotValCntH(ItmPerH, "overTm-ItemsPerH",
      TStr("Items (includes duplicates) per hour") + TmStr, "Time [hours]",
      "Total items (includes duplicates)");
  TGnuPlot::PlotValCntH(UniqItmH, "overTm-UniqItemsPerH",
      TStr("Unique Items (no duplicates) per hour") + TmStr, "Time [hours]",
      "Total items (includes duplicates)");
  TGnuPlot::PlotValCntH(LinkPerH, "overTm-LinksPerH",
      TStr("Links per hour") + TmStr, "Time [hours]", "Total links");
  TGnuPlot::PlotValCntH(QtsPerH, "overTm-QtsPerH",
      TStr("Quotes per hour") + TmStr, "Time [hours]", "Total quotes");
  TGnuPlot::PlotValCntH(BytesPerH, "overTm-BytesPerH",
      TStr("Bytes per hour") + TmStr, "Time [hours]", "Total bytes");
  TGnuPlot::PlotValCntH(PostPerH, "overTm-PostsPerH",
      TStr("Posts (good items) per hour") + TmStr, "Time [hours]",
      "Total Posts (good items)");
}

void TMakeQtBsFromSpinn3r::OnQuotesExtracted(
    const TQuoteExtractor& QuoteExtractor) {
  const TSecTm BegTm = TSecTm(2008, 1, 1); // beginning of time
...
PClustNet TClustNet::GetSubGraph(const int& MinQtWords, const int& MaxQtWords,
    const int& MinFq) const {
  TIntV NIdV;
...
void TClustNet::DelLongQts(const int& MxWrdLen) {
...
void TClustNet::MakeClusters(const TIntPrV& KeepEdgeV) {
  ...
  for (TEdgeI EI = BegEI(); EI < EndEI(); EI++) {
    const int ccid1 =
        NIdCcIdH.IsKey(EI.GetSrcNId()) ?
            NIdCcIdH.GetDat(EI.GetSrcNId()).Val : -1;
    const int ccid2 =
        NIdCcIdH.IsKey(EI.GetDstNId()) ?
            NIdCcIdH.GetDat(EI.GetDstNId()).Val : -1;
...
void TClustNet::KeepOnlyTree(const TIntPrV& KeepEdgeV) {
  ...
  for (int i = 0; i < KeepEdgeV.Len(); i++) {
    EdgeSet.AddKey(
        TIntPr(TMath::Mn(KeepEdgeV[i].Val1, KeepEdgeV[i].Val2),
...
void TClustNet::GetMergedClustQt(const TIntV& QtIdV, TQuote& NewQt) const {
  ...
  for (int c = 0; c < QtIdV.Len(); c++) {
    ...
    for (int t = 0; t < Q.GetTimes(); t++) {
      TmUrlCntH.AddDat(TPair<TSecTm, TInt>(Q.GetTm(t), Q.GetUrlId(t))) +=
          Q.GetCnt(t);
    }
    if (MxFq < Q.GetFq()) {
      MxFq = Q.GetFq();
  ...
  for (int u = 0; u < TmUrlCntH.Len(); u++) {
    NewQt.TmUrlCntV.Add(
        TQuote::TTmUrlCnt(TmUrlCntH.GetKey(u).Val1, TmUrlCntH.GetKey(u).Val2(),
            TmUrlCntH[u]()));
  }
  NewQt.TmUrlCntV.Sort();
}
...
int TClustNet::EvalPhraseClusters(const TIntPrV& KeepEdgeV,
  ...
  for (int cc = 0; cc < CnComV.Len(); cc++) {
    //if (CnComV[cc].NIdV.Len() < 1) { continue; } // TODO: BOOKMARK HERE ELEPHANTS
...
void TClustNet::ClustKeepSingleEdge(const int& MethodId,
    TIntPrV& KeepEdgeV) const {
  PNGraph G = TSnap::ConvertGraph < PNGraph
      > (TPt < TClustNet > ((TClustNet*) this));
  KeepEdgeV.Clr(false);
  ...
  for (TNodeI NI = BegNI(); NI < EndNI(); NI++) {
    ...
    for (int e = 0; e < NI.GetOutDeg(); e++) {
      ...
      if (MethodId == 1 && NI.GetOutNDat(e).GetFq() > BestVal) {
        EdgeToKeep = e;
        BestVal = NI.GetOutNDat(e).GetFq();
      }
      // keep link to longest variant
      if (MethodId == 2 && NI.GetOutNDat(e).GetStr().Len() > BestVal) {
        EdgeToKeep = e;
        BestVal = NI.GetOutNDat(e).GetStr().Len();
      }
      // keep link to shortest variant
      if (MethodId == 3 && NI.GetOutNDat(e).GetStr().Len() < BestVal
          || BestVal == 0) {
        EdgeToKeep = e;
        BestVal = NI.GetOutNDat(e).GetStr().Len();
      }
  ...
  if (MethodId == 1) {
    printf("Keep edge to most frequent quote:\n");
  }
  if (MethodId == 2) {
    printf("Keep edge to longest quote:\n");
  }
  if (MethodId == 3) {
    printf("Keep edge to shortest quote:\n");
  }
  if (MethodId == 4) {
    printf("Keep random edge:\n");
  }
  ...
  while (NIdOutDegH.Len() > 0 && NIdOutDegH[0] == 0) {
    for (int i = 0; i < NIdOutDegH.Len() && NIdOutDegH[i] == 0; i++) {
      ...
      for (int e = 0; e < NI.GetOutDeg(); e++) {
        ...
        ClustCntH.AddDat(NIdClustH.GetDat(NI.GetOutNId(e))).Val2 = NI.GetOutNId(
            e);
      }
      ClustCntH.SortByDat(false);
      if (ClustCntH.Len() > 0) {
        // point to cluster where we have most edges into
        KeepEdgeV.Add(TIntPr(NI.GetId(), ClustCntH[0].Val2)); // edge
        NIdClustH.AddDat(NI.GetId(), NIdClustH.GetDat(ClustCntH[0].Val2)); // cluster id
      }
...
void TClustNet::ClustGreedyRandom() const {
  ...
  for (TNodeI NI = BegNI(); NI < EndNI(); NI++) {
    if (NI.GetOutDeg() > 0) {
      EdgeH.AddDat(NI.GetId()) = NI.GetOutNId(
          TInt::Rnd.GetUniDevInt(NI.GetOutDeg()));
      if (NI.GetOutDeg() > 1) {
        for (int e = 0; e < NI.GetOutDeg(); e++) {
    ...
    while (NIdOutDegH.Len() > 0 && NIdOutDegH[0] == 0) {
      for (int i = 0; i < NIdOutDegH.Len() && NIdOutDegH[i] == 0; i++) {
        ...
        for (int e = 0; e < NI.GetOutDeg(); e++) {
          ...
          ClustCntH.AddDat(NIdClustH.GetDat(NI.GetOutNId(e))).Val2 =
              NI.GetOutNId(e);
        }
        ClustCntH.SortByDat(false);
        if (ClustCntH.Len() > 0) {
  ...
  while (true) {
    ...
    const int RndEdge = OutNIdV.GetDat(RndNId)[TInt::Rnd.GetUniDevInt(
        OutNIdV.GetDat(RndNId).Len())];
    const int id = EdgeH.GetKeyId(RndNId);
    IAssert(KeepEdgeV[id].Val1 == RndNId);
    if (KeepEdgeV[id].Val2 == RndEdge) {
      continue;
    } // same edge
...
void TClustNet::DrawNet(const TStr& OutFNm, const int& SaveTopN) const {
  ...
  for (int Comp = 0; Comp < TMath::Mn(SaveTopN, CnComV.Len()); Comp++) {
    if (CnComV[Comp].Len() < 5) {
      continue;
    }
    TPt < TNet > SubNet = TSnap::GetSubGraph(TPt < TNet > ((TClustNet*) this),
        CnComV[Comp].NIdV);
    printf("draw: %d nodes, %d edges\n", SubNet->GetNodes(),
        SubNet->GetEdges());
    FILE *F = fopen(TStr::Fmt("%s-c%02d.dot", OutFNm.CStr(), Comp).CStr(),
        "wt");
    ...
    for (TNet::TNodeI NI = SubNet->BegNI(); NI < SubNet->EndNI(); NI++) {
      fprintf(F, "  %d [label=\"%s (%d, %d)\"];\n", NI.GetId(),
          InsertLineBreaks(NI().GetStr(), 80).CStr(), NI().GetFq(),
          NI().GetUrls());
    }
    ...
    fprintf(F, "}\n");
    fclose(F);
    TGraphViz::DoLayout(TStr::Fmt("%s-c%02d.dot", OutFNm.CStr(), Comp),
        TStr::Fmt("%s-c%02d.ps", OutFNm.CStr(), Comp), gvlDot);
...
void TClustNet::DumpNodes(const TStr& OutFNm, const int& SaveTopN) const {
  ...
  for (int i = 0; i < FqNIdV.Len(); i++) {
    ...
    fprintf(F, "%d\t%d\t%s\n", Q.GetFq(), NIdCompSzH.GetDat(FqNIdV[i].Val2).Val,
        Q.GetStr().CStr());
  }
}
...
void TClustNet::DumpClustersByFq(const TStr& OutFNm, const int& MinClustSz,
    const int& SaveTopN) const {
  TCnComV CnComV;
  TSnap::GetWccs(GetThis(), CnComV);
  CnComV.Sort(false);
  TIntPrV FqClustV;
  ...
  for (int c = 0; c < TMath::Mn(SaveTopN, FqClustV.Len()); c++) {
...
void BuildPhraseInvertIdx(const PQuoteBs& QtBs, TStrHash<TInt>& WordIdH,
    THash<TInt, TIntV>& WIdQtIdVH) {
...
bool IsLinkPhrases(const PQuoteBs& QtBs, const int& QtN1, const int& QtN2,
    TStrHash<TInt>& WordIdH, THash<TInt, TIntV>& QtToWordIdVH) {
  ...
  const int Overlap = TQuoteBs::LongestCmnSubSq(WIdV1, WIdV2, idx1, idx2,
      SkipTy);
  bool DoMerge = false;
  if (ShortLen <= 5 && Overlap == ShortLen /*&& SkipTy==0*/) {
    DoMerge = true;
  } // full overlap, no skip
...
PClustNet TClustNet::GetFromQtBs(const PQuoteBs& QtBs, int MinQtFq,
    int MnWrdLen) {
...
//        }
    ...
    if (qt % 1000 == 0 || qt == QtBsLen - 1) {
      printf("\r%d  %s    allCands: %d    allLinks: %d    nodes:%d", qt,
          ExeTm.GetStr(), AllCand, AllLinks, Net->GetNodes());
      if (qt % 100000 == 0 || qt == QtBsLen - 1) {
        TGnuPlot::PlotValCntH(CandLenH, "qtNet-NCand",
            "Number of candidate links for each quote (candidate has at most 2*words)",
            "ClustNet: number of candidate quotes to create link to", "Count",
            gpsLog);
        TGnuPlot::PlotValCntH(LinkLenH, "qtNet-NLink",
            "Number of links for each quote (candidate has at most 2*words)",
...
bool TQuoteLoader::LoadItem(TXmlLx& XmlLx) {
  ...
  try {
    ...
    PubTm = TSecTm(atoi(T.GetSubStr(0, 3).CStr()),
        atoi(T.GetSubStr(5, 6).CStr()), atoi(T.GetSubStr(8, 9).CStr()),
        atoi(T.GetSubStr(11, 12).CStr()), atoi(T.GetSubStr(14, 15).CStr()),
        atoi(T.GetSubStr(17, 18).CStr()));
...
bool TQuoteLoader::Next() {
  if (SIn.Empty() || SIn->Eof()) {
    printf("  new file");
    if (!FFile.Next(CurFNm)) {
      return false;
    }
    printf(" %s\n", CurFNm.GetFMid().CStr());
    SIn =
        TZipIn::IsZipExt(CurFNm.GetFExt()) ?
            PSIn(new TZipIn(CurFNm)) : PSIn(new TFIn(CurFNm));
...
void TQuoteLoader::ProcessPosts(const bool& IsXml, int LoadN) {
  ...
  for (int f = 1; FFile.Next(CurFNm); f++) {
    ...
    if (IsXml) {
      ...
      StartProcFile(CurFNm);
      for (FilePostCnt = 0; XmlLx.GetSym() != xsyEof;
          FilePostCnt++, PostCnt++) {
        if (!(XmlLx.Sym == xsySTag && XmlLx.TagNm == "post")) {
          while (XmlLx.GetSym() != xsyEof
              && !(XmlLx.Sym == xsySTag && XmlLx.TagNm == "post")) {
          }
          if (XmlLx.Sym == xsyEof) {
            break;
          }
...
bool TMemesDataLoader::LoadNext() {
  ...
  TSIn& SIn = *SInPt;
  TChA Line;
  while (SIn.GetNextLn(Line) && (Line.Empty() || Line[0] != 'P')) {
    LineCnt++;
  }
  LineCnt++;
  if (Line.Empty()) {
  ...
  IAssert(SIn.GetNextLn(Line) && (!Line.Empty()) && Line[0] == 'T');
  LineCnt++;
...
void TMemesDataLoader::Dump(const bool& DumpAll) const {
  printf("%s\n  %s\tQ:%d\tL:%d\n", PostUrlStr.CStr(),
      PubTm.GetYmdTmStr().CStr(), MemeV.Len(), LinkV.Len());
  if (DumpAll) {
    for (int i = 0; i < MemeV.Len(); i++) {
      printf("%s\n", MemeV[i].CStr());
    }
    for (int i = 0; i < LinkV.Len(); i++) {
      printf("%s\n", LinkV[i].CStr());
    }
  }
}
