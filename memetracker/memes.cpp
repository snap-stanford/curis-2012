...
void TClustNet::DumpClustersByFq(const TStr& OutFNm, const int& MinClustSz, const int& SaveTopN) const {
  TCnComV CnComV;
  TSnap::GetWccs(GetThis(), CnComV);
  CnComV.Sort(false);
  TIntPrV FqClustV;
  for (int Comp = 0; Comp < CnComV.Len(); Comp++) {
    const TIntV& NIdV = CnComV[Comp].NIdV;
    if (NIdV.Len() < MinClustSz) { continue; }
    int SumFq=0;
    for (int n = 0; n < NIdV.Len(); n++) {
      SumFq += GetNDat(NIdV[n]).GetFq(); }
...
bool TMemesDataLoader::LoadNext() {
  ...
  TSIn& SIn = *SInPt;
  TChA Line;
  while (SIn.GetNextLn(Line) && (Line.Empty() || Line[0]!='P')) { LineCnt++; }
  LineCnt++;
  if (Line.Empty()) {
