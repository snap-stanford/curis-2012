    ...
    (new Request.JSON({
        ...
        onSuccess: function (b) {
            ...
            for (var i = 0; i < labels.length; ++i) {
              ...
              var folder_index = Math.floor(labels[i] / 1000);
              quote_link.setAttribute("href", ClusterPage + "id=" + labels[i]);
              quote_link.appendChild(document.createTextNode(quotes[i]));
              td_quote.appendChild(quote_link);
...
function init() {
    (function () {
        ...
        if (MainDesc.indexOf(Weekly) != -1) {
          ...
          Type = Monthly;
          StartDate = new Date(2012, 2, 1, 0, 0, 0, 0);
