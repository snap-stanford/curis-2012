(function () {
    ...
    $jit.StreamChart = new m({
        ...
        setupLabels: function () {
            var a = this.filterArray,
                c = this.delegate.graph,
                b = c.getNode(this.delegate.root),
                d = {}, e = {}, f = this.canvas.getCtx();
            this.canvas.getSize();
            b.eachAdjacency(function (a) {
                var a = a.nodeTo,
                    b = a.pos.getc(!0),
                    f = a.getData("dimArray"),
                    g = a.getData("stringArray");
                if (a.getData("prev") && c.getByName(a.getData("next"))) for (var h = 0, j = g.length; h < j; ++h) {
                    var i = g[h].trim(),
                        b = f[h][0],
                        m = f[h][1];
                    a.getData("left-pos");
                    a.getData("right-pos");
                    if (!(i in d) || d[i] < b || d[i] < m) {
                        if (b > m) {
                            d[i] = b;
                            var b = a.getData("left-pos"),
                                n = 2 * h,
                                b = [(b[n][0] + b[n + 1][0]) / 2, (b[n][1] + b[n + 1][1]) / 2]
                        }
                        else b = a.getData("right-pos"), n = 2 * h, b = [(b[n][0] + b[n + 1][0]) / 2, (b[n][1] + b[n + 1][1]) / 2], d[i] = m;
                        e[i] = b
                    }
                }
            });
            ...
            f.font = "13px Helvetica";
            for (var b = 0, g = a.length; b < g; ++b) {
                var j = e[a[b]];
                //var i = h(a[b] + "_name").innerHTML.split(" ")[0];
                f.fillStyle = f.strokeStyle = "white";
                //f.fillText(i, j[0], j[1])
                //alert(j);
                f.fillText(a[b], j[0], j[1])
            }
        },
        select: function (a, c, b) {
            if (this.config.selectOnHover) {
