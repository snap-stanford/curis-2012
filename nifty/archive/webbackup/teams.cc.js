...
}, filteredTeams = Object.keys(TeamData);
...
function init() {
    ...
    function r(a) {
        ...
        k.innerHTML = b[1] + " - " + o[1];
        (new Request.JSON({
            url: "data/matches/" + a.id + "/part.json",
            method: "get",
    ...
    (function () {
        for (var a in TeamData) $(a).innerHTML = TeamData[a].matches.map(function (a, c) {
            ...
            return "<div id='" + b + "' class='match'><div class='image-match' style='background-image:url(data/matches/" + b + "/match.png)'></div><div class='against-name'><span class='" + g.toLowerCase() + "'>" + g + "</span> vs. " + $(c.against + "_name").innerHTML + "</div></div>"
        }.bind(this, a)).join("")
    })();
    ...
    (function () {
        ...
        $("all-teams").addEvent("click", function (b) {
            b.stop();
            a.busy || (c.removeClass("selected"), this.addClass("selected"), filteredTeams = Object.keys(TeamData), a.filter(filteredTeams, {
                onComplete: function () {
                    a.setupLabels()
                }
            }))
        });
        $("teams-qf").addEvent("click",

        function (b) {
            a.busy || (b.stop(), c.removeClass("selected"), this.addClass("selected"), filteredTeams = Object.keys(TeamData).filter(function (a) {
                return 3 < TeamData[a].matches.length
            }), a.filter(filteredTeams, {
                onComplete: function () {
                    a.setupLabels()
                }
            }))
        });
        $("teams-nqf").addEvent("click", function (b) {
            a.busy || (b.stop(), c.removeClass("selected"), this.addClass("selected"), filteredTeams = Object.keys(TeamData).filter(function (a) {
                return 3 >= TeamData[a].matches.length
            }), a.filter(filteredTeams, {
                onComplete: function () {
                ...
                a = new $jit.StreamChart({
                    ...
                    Tips: {
                        ...
                        onShow: function (a, b, c) {
                            ...
                            a.innerHTML = "<b>" + d + "</b> tweets for <b>" + $(b.name.trim() + "_name").innerHTML + "</b> on " + [c[1], c[2], c[0]].join("/")
                        }
                    },
                    Events: {
                        enable: !0,
