...
void TQuoteBs::ShowPathInfo(int CId, int srcQt, int dstQt, PClustNet& Net, TStr& PathFNm) {
	...
	while (curQt != srcQt) {
		...
		fprintf(FPath, "src: TmMed = %04lf\tTmDev = %04lf\tCnt = %d\t %s\n", curQ.GetTmMed(), curQ.GetTmDev(), curQ.GetFq(), curQ.GetStr().CStr());
		fprintf(FPath, "dst: TmMed = %04lf\tTmDev = %04lf\tCnt = %d\t %s\n\n", nextQ.GetTmMed(), nextQ.GetTmDev(), nextQ.GetFq(), nextQ.GetStr().CStr());
		curQt = nextQt;
	}
	fclose(FPath);
}
...
void TQuoteBs::ElimBadBkt(TStr& BlackListFNm, double BktThresh, THash<TMd5Sig, TIntV>& ShglQtIdVH, THash<TInt, TIntV>& QtToWordIdVH) {
  ...
  for (TFIn FIn(BlackListFNm); FIn.GetNextLn(Ln); ) {
	...
	for (int i = 0; i < ShglQtIdVH.Len(); i++) {
		...
		if (QtIdV.Len() > 500) { // The hash table bucket cannot be too big
			ShglQtIdVH[i].Clr(false);
			continue;
		}

		// Judge the wellness of the bucket by weighted pairwise quote distance
		...
		// If the average quotes distance is greater than a threshold, the bucket is bad
		if (dist > BktThresh)
			ShglQtIdVH[i].Clr(false);
	}
}
	...
	for (int i = 0; i < ShglQtIdVH.Len(); i++) {
		...
		for (int qt1 = 0; qt1 < QtV.Len(); qt1++) 
			for (int qt2 = qt1 + 1; qt2 < QtV.Len(); qt2++) {
				...
				if (IsLinkPhrases(Sm, Lg, QtToWordIdVH)) {
  ...
  NSkip = 0; 	fileCnt = 0;
	...
	for (int i = 0; i < filelist.Len(); i++) {
		...
		while (Memes.LoadNext()) {
			...
			if (Memes.MemeV.Len() >= 1 && Memes.PubTm >= MinTm && Memes.PubTm <= MaxTm) {
				AddQuote(Memes.MemeV, Memes.LinkV, Memes.PostUrlStr, Memes.PubTm, MinWrdLen);
			}
...
void TQuoteBs::DumpQuoteClusters(int MinWrdLen, int MinClustFq, bool SkipUrl, bool FlashDisp, TStr Pref) {
  ...
  TIntV QtIdV;
  GetQtIdVByFq(QtIdV, MinWrdLen, MinClustFq, false); QtIdV.Del(100, QtIdV.Len()-1);
  SaveQuotes(QtIdV, Pref+"-quotes.txt");
  SaveClusters(Pref, true); 
